...
void Menu_enter_mask(int** grid_existant,int size){
    ...
    mask_applied = print_pad_mask(mask, grid_existant, size);
    if(auto_possible(mask_applied,size)) {
    ...
    {
        printf("Your mask makes an unsolvable grid. Press any key to go back to main menu.\n  ->");
        getch();
    }
}
